...
void DesignDirectory::set_dir_path(const QString &dir_path) {
    ...
    if (updateDatabase(d_path)) {
        std::cout << d_path.toStdString() << std::endl;
...
bool DesignDirectory::initializeDatabase() {
    ...
    QSqlQuery query;
    if (!query.exec("CREATE TABLE IF NOT EXISTS config (id INTEGER PRIMARY KEY, dir_path TEXT)")) {
...
bool DesignDirectory::updateDatabase(const QString &dir_path) {
    ...
    if (!query.exec()) {
        qWarning() << "Failed to update directory path: " << query.lastError().text();
        return false;
    }
    return true;
}
